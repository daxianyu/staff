...
import { lessonDayChange } from '@/services/modules/tools';
import { getAllCampus } from '@/services/modules/academics';
import type { Campus } from '@/services/modules/academics';
import { CheckCircleIcon, ExclamationCircleIcon } from '@heroicons/react/24/outline';

interface LessonDayChangeModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function LessonDayChangeModal({ isOpen, onClose }: LessonDayChangeModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await lessonDayChange({
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="调整课程日期">
