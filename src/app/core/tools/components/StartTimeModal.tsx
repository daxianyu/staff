...
import { addStartTime, deleteStartTime, getStartTime } from '@/services/modules/tools';
import { getAllCampus } from '@/services/modules/academics';
import type { Campus } from '@/services/modules/academics';
import type { StartTimeRecord } from '@/services/modules/tools';
...
export default function StartTimeModal({ isOpen, onClose }: StartTimeModalProps) {
  ...
  const [records, setRecords] = useState<StartTimeRecord[]>([]);
  ...
  const loadRecords = async () => {
    try {
      setLoading(true);
      const response = await getStartTime();
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      const response = await addStartTime({
  ...
  const handleDelete = async (recordId: number) => {
    ...
    try {
      const response = await deleteStartTime({ record_id: recordId });
      if (response.code === 200) {
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="开学时间管理" maxWidth="xl">
      <div className="space-y-4">
        ...
        {showAddForm && (
          <form onSubmit={handleAdd} className="bg-gray-50 p-4 rounded-md space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                开学日期 <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
