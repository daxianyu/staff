...
import { getStudentFullness } from '@/services/auth';
import { AcademicCapIcon, ExclamationTriangleIcon, CalendarIcon, ClockIcon } from '@heroicons/react/24/outline';

interface StudentFullnessData {
  student_name: string;
  campus_names: string;
  hours: string;
...
export default function StudentFullnessPage() {
  const { hasPermission } = useAuth();
  const [data, setData] = useState<StudentFullnessData[]>([]);
  ...
  const [sortField, setSortField] = useState<keyof StudentFullnessData>('fullness');
  ...
  const loadData = async (month: number) => {
    try {
      setLoading(true);
      const response = await getStudentFullness(month);
  ...
  // 排序处理
  const handleSort = (field: keyof StudentFullnessData) => {
    if (sortField === field) {
  ...
  const parseHours = (timeStr: string): number => {
    const hoursMatch = timeStr.match(/(\d+(?:\.\d+)?)小时/);
    const minutesMatch = timeStr.match(/(\d+)分钟/);
  ...
  const filteredAndSortedData = data
    ...
    .sort((a, b) => {
      ...
      // 对于特殊字段进行数值转换
      if (sortField === 'hours') {
        aValue = parseHours(aValue);
        bValue = parseHours(bValue);
      } else if (sortField === 'fullness') {
  ...
  const totalHours = data.reduce((sum, item) => sum + parseHours(item.hours), 0);
  const avgFullness = data.length > 0 
    ? data.reduce((sum, item) => sum + parseFloat(item.fullness), 0) / data.length 
    : 0;
  const highFullnessCount = data.filter(item => parseFloat(item.fullness) > 1).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* 页面标题 */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">学生学习饱和度</h1>
          <p className="mt-2 text-sm text-gray-600">学生月度学习时间饱和度分析</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500">平均饱和度</p>
                <p className="text-2xl font-semibold text-gray-900">{avgFullness.toFixed(2)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500">高饱和度学生</p>
                <p className="text-2xl font-semibold text-gray-900">{highFullnessCount}</p>
                <p className="text-xs text-gray-500">饱和度  1.0</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th 
                      className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                      onClick={() => handleSort('campus_names')}
                    >
                      <div className="flex items-center gap-1">
                        校区
                        {sortField === 'campus_names' && (
                          <span className="text-blue-600">
                            {sortDirection === 'asc' ? '↑' : '↓'}
                          </span>
                        )}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedData.length === 0 ? (
                    ...
                    paginatedData.map((item, index) => {
                      const fullness = parseFloat(item.fullness);
                      const fullnessColor = fullness > 1.2 ? 'text-red-600' : fullness > 0.8 ? 'text-orange-600' : 'text-green-600';
                      const fullnessBgColor = fullness > 1.2 ? 'bg-red-100' : fullness > 0.8 ? 'bg-orange-100' : 'bg-green-100';
                      ...
                      return (
                        <tr key={`${item.student_name}-${index}`} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <div className="flex items-center">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${fullnessBgColor} ${fullnessColor}`}>
                                {fullness.toFixed(3)}
                              </span>
                              <div className="ml-2 w-16 bg-gray-200 rounded-full h-2">
                                <div
                                  className={`h-2 rounded-full ${
                                    fullness > 1.2 ? 'bg-red-600' : fullness > 0.8 ? 'bg-orange-600' : 'bg-green-600'
                                  }`}
