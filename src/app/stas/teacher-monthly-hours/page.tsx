...
import { getTeachingHoursOverview } from '@/services/auth';
...
interface TeachingHoursData {
  ...
  teacher_data_list: Record<string, CampusData>;
}

export default function TeacherMonthlyHoursPage() {
  ...
  const loadData = async (month: number) => {
    try {
      setLoading(true);
      const response = await getTeachingHoursOverview(month);
      if (response.code === 200) {
        const responseData = response.data as TeachingHoursData;
        setData(responseData);
        if (responseData?.teacher_data_list) {
          const campusIds = Object.keys(responseData.teacher_data_list);
  ...
  const currentCampusData = data && selectedCampus ? data.teacher_data_list[selectedCampus] : null;

  // 分页计算
  const totalItems = currentCampusData?.data.length || 0;
  const totalPages = Math.ceil(totalItems / pageSize);
  const startIndex = (currentPage - 1) * pageSize;
  const endIndex = startIndex + pageSize;
  const paginatedData = currentCampusData?.data.slice(startIndex, endIndex) || [];
  ...
  const prepareExportData = () => {
    ...
    // 为每个校区创建一个sheet
    const sheets = Object.entries(data.teacher_data_list).map(([campusId, campusData]) => {
      const teachers = campusData.data.map(teacher => ({
        教师姓名: teacher.name,
        校区: campusData.name,
        授课时数: teacher.hours,
        双倍课时: teacher.double_hours
      }));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* 页面标题 */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">教师月度授课时数</h1>
          <p className="mt-2 text-sm text-gray-600">查看教师每月授课时间统计，包括双倍课时</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500">双倍课时</p>
                  <p className="text-2xl font-semibold text-gray-900">{data.total.sum_double}</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-semibold text-gray-900">
                    {Object.keys(data.teacher_data_list).length}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500">当前校区教师</p>
                  <p className="text-2xl font-semibold text-gray-900">
        ...
        <div className="bg-white rounded-lg shadow mb-6 p-6">
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              {/* 校区选择 */}
              {data && Object.keys(data.teacher_data_list).length > 0 && (
                <div className="flex items-center gap-4">
                  ...
                  >
                    {Object.entries(data.teacher_data_list).map(([campusId, campusData]) => (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedData.map((teacher, index) => {
                    ...
                    const percentage = currentCampusData.sum_hour > 0 
                      ? (Number(teacher.hours) / currentCampusData.sum_hour * 100) 
                      : 0;
                    ...
                    return (
                      <tr key={teacher.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          <button
                            onClick={() => router.push(`/schedule?staffId=${teacher.id}`)}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            ...
                            <span className="font-semibold">{teacher.hours}</span>
                            <span className="text-gray-500 ml-1">小时</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            <ClockIcon className="h-4 w-4 text-green-400 mr-1" />
                            <span className="font-semibold">{teacher.double_hours.toFixed(1)}</span>
                            <span className="text-gray-500 ml-1">小时</span>
                          </div>
                        </td>
                      </tr>
