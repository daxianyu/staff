...
import { 
  getWithdrawalExamTable, 
  downloadWithdrawalExamTable,
  updateWithdrawalExamStatus,
  deleteWithdrawalExam,
  type WithdrawalExamItem,
...
export default function WithdrawalOverviewPage() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<WithdrawalExamItem | null>(null);
  ...
  const handleDownload = async () => {
    try {
      const result = await downloadWithdrawalExamTable();
      if (result.code === 200 && result.data?.file_path) {
        // 创建下载链接
        const link = document.createElement('a');
        link.href = result.data.file_path;
        link.download = `withdrawal_exam_${new Date().getTime()}.xlsx`;
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteWithdrawalExam(selectedRecord.record_id);
      if (result.code === 200) {
        alert('删除成功');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Withdrawal Overview</h1>
          <p className="text-gray-600 mt-1">退考信息管理</p>
        </div>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        确定要删除学生 <strong>{selectedRecord.student_name}</strong> 的退考申请吗？此操作不可撤销。
                      </p>
                    </div>
                  </div>
                </div>
